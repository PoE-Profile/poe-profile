...
class PoeTree extends Command
{
    ...
    public function handle()
    {
        ...
        $this->updateAlternateTree();
    }

    function updateAlternateTree() {
        $userAgent="Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:97.0) Gecko/20100101 Firefox/97.0";
        $response = Http::withHeaders(['User-Agent' => $userAgent])->get('https://www.pathofexile.com/fullscreen-passive-skill-tree/alternate');
        $html = $response->body();
        $tempHtml = trim(preg_replace('/\s\s+/', '', $html));
        ///set latest_nodes.json
        $nodes='';
        ...
        //     $r->item(0)->parentNode->removeChild($r->item(0));
        // }
        \Storage::put('passive-skill-tree-alternate.html', (string) $dom->saveHTML());
        $this->info("update storage cache for passive-skill-tree-alternate.html ");
    }
}
